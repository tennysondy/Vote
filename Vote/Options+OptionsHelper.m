...
+ (void)updateVotersWithOptions:(NSDictionary *)details ofVote:(VotesInfo *)aVote withContext:(NSManagedObjectContext *)context
{
    ...
    for (Options *elem in results) {
        //modify the voters
        NSMutableDictionary *updatedDict = [elem.voters mutableCopy];
        [updatedDict removeAllObjects];
        elem.voters = [updatedDict copy];
    }
    Options *aOption = nil;
    for (id key in details) {
        ...
        if ([results count] == 1) {
            ...
            [updatedDict removeAllObjects];
            if ((NSNull *) [details objectForKey:key] != [NSNull null]) {
                updatedDict = (NSMutableDictionary *)[details objectForKey:key];
            }
            aOption.voters = [updatedDict copy];
            NSLog(@"order:%@, voters:%@",aOption.order, aOption.voters);
        } else {
