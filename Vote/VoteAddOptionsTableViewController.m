...
@property (strong, nonatomic) NSString *optionCity;
...
- (IBAction)addOptions:(id)sender {
    ...
    if ([NSString checkWhitespaceAndNewlineCharacter:self.optionName.text] == YES) {
        UIAlertView *av = [[UIAlertView alloc] initWithTitle:@"名称不可为空" message:nil delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
        [av show];
        return;
    }
    if ([self.optionCatrgory length] <= 0) {
        UIAlertView *av = [[UIAlertView alloc] initWithTitle:@"分类不可为空" message:nil delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
        [av show];
        return;
    }
...
- (UITextField *)optionName
{
    if (_optionName == nil) {
        ...
        if ([self.defaultName length] > 0) {
            _optionName.text = self.defaultName;
        }
