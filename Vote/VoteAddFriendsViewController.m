...
- (IBAction)search:(id)sender {
    ...
    if (self.friendNameTextField.text.length > 0) {
        ...
        [manager GET:getInfoURL parameters:para success:^(AFHTTPRequestOperation *operation, id responseObject){
            ...
            if ((NSNull *)[responseObject objectForKey:SERVER_USERNAME] == [NSNull null]) {
                UIAlertView *av = [[UIAlertView alloc] initWithTitle:@"查找错误" message:@"该用户名不存在，请检查拼写后重新搜索" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
                [av show];
            } else {
