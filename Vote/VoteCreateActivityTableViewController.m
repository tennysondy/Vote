...
@interface VoteCreateActivityTableViewController () <UITextFieldDelegate, UIPickerViewDataSource, UIPickerViewDelegate, UIAlertViewDelegate, UITextViewDelegate>
{
    ...
    NSArray *sortArray;
    NSMutableDictionary *lastOption;
}
...
- (void)viewDidLoad
{
    ...
    sectionTitles = [[NSMutableArray alloc] initWithObjects:@"活动主题", @"活动描述", @"活动配图", @"截止时间", @"添加好友", @"公众，匿名，多选设置", @"候选列表(长按选项可编辑)", nil];
    ...
    lastOption = [[NSMutableDictionary alloc] initWithCapacity:2];
...
- (IBAction)publish:(id)sender {
    ...
    if ([NSString checkWhitespaceAndNewlineCharacter:self.activityDesc.text] == YES) {
        NSString *title = @"活动创建失败";
        NSString *msg = @"描述不可为空";
        NSString *btnTitle = @"确定";
        UIAlertView *av = [[UIAlertView alloc] initWithTitle:title message:msg delegate:nil cancelButtonTitle:btnTitle otherButtonTitles:nil];
    ...
    if ([self.thePublic boolValue] == YES && [self.anonymous boolValue] == NO) {
        NSString *title = @"活动创建失败";
        NSString *msg = @"若活动面向公众，则投票必须设置为匿名";
        NSString *btnTitle = @"确定";
        UIAlertView *av = [[UIAlertView alloc] initWithTitle:title message:msg delegate:nil cancelButtonTitle:btnTitle otherButtonTitles:nil];
        [av show];
        return;
    ...
    if ([choice unsignedIntegerValue] > [self.optionsList count]) {
        NSString *title = @"活动创建失败";
        NSString *msg = @"选项个数必须大于或等于最大可选数";
        NSString *btnTitle = @"确定";
        UIAlertView *av = [[UIAlertView alloc] initWithTitle:title message:msg delegate:nil cancelButtonTitle:btnTitle otherButtonTitles:nil];
        [av show];
        return;
    }
    [self publishTheVote:sender];
}

- (void)publishTheVote:(id)sender
{
    //显示loading动画
...
- (NSMutableArray *)createOptions
{
    ...
    for (NSUInteger i = 0; i < [self.optionsList count]; i++) {
        ...
        if ((NSNull *)[element objectForKey:SERVER_OPTIONS_NAME] != [NSNull null]) {
            NSString *name = [element objectForKey:SERVER_OPTIONS_NAME];
            [aOption setObject:name forKey:SERVER_OPTIONS_NAME];
        } else {
        ...
        if ((NSNull *)[element objectForKey:SERVER_OPTIONS_BUSINESS_ID] != [NSNull null]) {
            NSNumber *businessId = [element objectForKey:SERVER_OPTIONS_BUSINESS_ID];
            [aOption setObject:businessId forKey:SERVER_OPTIONS_BUSINESS_ID];
        } else {
        ...
        if ((NSNull *)[element objectForKey:SERVER_OPTIONS_CATEGORY] != [NSNull null]) {
            NSString *category = [element objectForKey:SERVER_OPTIONS_CATEGORY];
            [aOption setObject:category forKey:SERVER_OPTIONS_CATEGORY];
        } else {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    if (section < 5) {
        ...
        if ([self.optionsList count] == 0) {
            ...
            NSLog(@"count:%lu", (unsigned long)[self.optionsList count]);
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if (section == 6) {
        ...
        addButton.tintColor = UIColorFromRGB(0x1AB5FF);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0 && indexPath.row == 0) {
        ...
        if ([cell.contentView viewWithTag:CATVC_DESCRIPTION_TEXTVIEW_TAG] == nil) {
            [cell.contentView addSubview:self.activityDesc];
        }
        ...
        if ([self.imageAttr count] == 0) {
            ...
            UIImage *image = [UIImage imageNamed:[self.imageAttr objectForKey:@"name"]];
        ...
        UILabel *title = [[UILabel alloc] initWithFrame:CGRectMake(15.0, 10.0, 110.0, 30.0)];
        ...
        CGFloat nameHeight;
        if ((NSNull *)[data objectForKey:SERVER_OPTIONS_ADDRESS] == [NSNull null]) {
            nameHeight = FRAME_HEIGHT(cell.contentView) - 20.0;
        } else {
            nameHeight = [NSString calculateTextHeight:name font:nameFont width:width];
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 6 && [self.optionsList count] == 0) {
        [self performSegueWithIdentifier:@"Add Options" sender:indexPath];
    }
...
- (UITextView *)activityDesc
{
    if (_activityDesc == nil) {
        CGRect rect = CGRectMake(15, 0, 290, 110);
        _activityDesc = [[UITextView alloc] initWithFrame:rect];
        _activityDesc.tag = CATVC_DESCRIPTION_TEXTVIEW_TAG;
        _activityDesc.font = [UIFont systemFontOfSize:15.0];
        _activityDesc.autocorrectionType = UITextAutocorrectionTypeNo;
        _activityDesc.textAlignment = NSTextAlignmentLeft;
        _activityDesc.keyboardType = UIKeyboardTypeDefault;
        _activityDesc.returnKeyType = UIReturnKeyDone;
        _activityDesc.scrollEnabled = NO;
        //_description.layer.borderColor = [[UIColor blackColor] CGColor];
        //_description.layer.borderWidth = 1.0;
        _activityDesc.delegate = self;
    }
...
- (UITextField *)deadline
{
    if (_deadline == nil) {
        ...
        [cancelButton setTitleTextAttributes:@{NSForegroundColorAttributeName: [UIColor blackColor], NSFontAttributeName:[UIFont systemFontOfSize:15.0f]} forState:UIControlStateNormal];
        //定义完成按钮
        ...
        [doneButton setTitleTextAttributes:@{NSForegroundColorAttributeName: [UIColor blackColor], NSFontAttributeName:[UIFont systemFontOfSize:15.0f]} forState:UIControlStateNormal];
        //在toolBar上加上这些按钮
...
- (void)addOptionWithData:(NSDictionary *)data
{
    ...
    [lastOption setObject:[data objectForKey:SERVER_OPTIONS_NAME] forKey:SERVER_OPTIONS_NAME];
    [lastOption setObject:[data objectForKey:SERVER_OPTIONS_CATEGORY] forKey:SERVER_OPTIONS_CATEGORY];
    NSLog(@"lastOption: %@", lastOption);
}
...
- (void)addParticipants:(NSMutableArray *)participants
{
    ...
    NSLog(@"self.participants:%@", self.participants);
    ...
    NSDictionary *name = @{SERVER_USERNAME:username, SERVER_SCREENNAME:screenname};
...
- (void)handleLongPress:(UILongPressGestureRecognizer *)sender
{
    ...
    if(UIGestureRecognizerStateBegan == longPressGR.state) {
        ...
        if(indexPath != nil && indexPath.section == 6 && [self.optionsList count] > 0) {
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([segue.identifier isEqualToString:@"Add Options"]) {
        ...
        if ([lastOption count] > 0) {
            vc.defaultName = [lastOption objectForKey:SERVER_OPTIONS_NAME];
            vc.defaultCategory = [lastOption objectForKey:SERVER_OPTIONS_CATEGORY];
        }
        ...
        if ([self.participants count] > 0) {
            ...
            NSString *screenname = aUser.screenname;
            NSDictionary *name = @{SERVER_USERNAME:username, SERVER_SCREENNAME:screenname};
            [self.participants removeObject:name];
