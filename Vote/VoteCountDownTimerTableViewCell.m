...
- (void)awakeFromNib
{
    ...
    if (!self.separator) {
        rect = CGRectMake(10.0, 69.5, 310.0, 0.5);
...
- (void)setAnonymous:(NSNumber *)anonymous
{
    ...
    if ([anonymous boolValue] == YES) {
        ...
        self.anonymousLabel.backgroundColor = UIColorFromRGB(0x674EA7);
    } else {
        self.anonymousLabel.text = @"公";
        self.anonymousLabel.backgroundColor = UIColorFromRGB(0x00CC99);
    }
}
...
- (void)setThePublic:(NSNumber *)thePublic
{
    ...
    if ([thePublic boolValue] == YES) {
        ...
        self.anonymousLabel.backgroundColor = UIColorFromRGB(0xEF4D25);
    }
}
...
- (void)updateCounter
{
    ...
    if ([[self.endTime earlierDate:now] isEqualToDate:self.endTime]) {
        ...
        if (components.day > 0) {
            self.timerLabel.text = [NSString stringWithFormat:@"还有%ld天", (long)[components day]];
        } else if (components.hour > 0 || components.minute > 0) {
            NSString *timerString = [NSString stringWithFormat:@"还有%02ld:%02ld", (long)[components hour], (long)[components minute]];
            
            self.timerLabel.text = [NSString stringWithFormat:@"%@", timerString];
        } else {
            NSString *timerString = [NSString stringWithFormat:@"还有%02ld秒", (long)[components second]];
