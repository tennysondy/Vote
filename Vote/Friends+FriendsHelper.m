...
+ (void)insertFriendsToDatabaseWithData:(NSDictionary *)data withManagedObjectContext:(NSManagedObjectContext *)context withQueue:(NSOperationQueue *)queue
{
    ...
    [Friends downloadHeadImageOfFriend:aFriend withUrl:aFriend.originalHeadImageUrl withManagedObjectContext:context withQueue:queue];
    [Friends downloadHeadImageOfFriend:aFriend withUrl:aFriend.mediumHeadImageUrl withManagedObjectContext:context withQueue:queue];
...
+ (void)modifyFriends:(Friends *)aFriend withData:(NSDictionary *)data withManagedObjectContext:(NSManagedObjectContext *)context withQueue:(NSOperationQueue *)queue
{
    ...
    if ([Friends checkImageExistedOfFriend:aFriend] == NO) {
        aFriend.headImageLastUpdateTag = [NSNumber numberWithInteger:0];
    }
...
+ (void)downloadHeadImageOfFriend:(Friends *)aFriend withUrl:(NSString *)url withManagedObjectContext:(NSManagedObjectContext *)context withQueue:(NSOperationQueue *)queue
{
    ...
    [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if ([url isEqualToString:aFriend.originalHeadImageUrl]) {
            ...
            if ([Friends checkStoreDirectoryforFriend:aFriend ofDir:TEMPORARY]) {
                aFriend.thumbnailsHeadImagePath = [Friends saveImage:tmpImage ofFriend:aFriend withName:THUMBNAILS_HEAD_IMAGE_NAME andType:IMAGE_TYPE inDir:TEMPORARY];
            }
        }
...
+ (void)deleteFriendsOnServer:(NSString *)friendName withManagedObjectContext:(NSManagedObjectContext *)context
{
    NSUserDefaults *ud= [NSUserDefaults standardUserDefaults];
    NSString *username = [ud stringForKey:USERNAME];
    NSString *url = [[NSString alloc] initWithFormat:@"http://115.28.228.41/vote/friend.php"];
    NSNumber *friendAction = [[NSNumber alloc] initWithInt:DELETE_FRIEND_REQUEST];
    NSDictionary *parameters = @{SERVER_USERNAME: username, SERVER_FRIEND_NAME: friendName, SERVER_FRIEND_ACTION: friendAction};
    NSLog(@"URL parameters = %@", parameters);
...
+ (BOOL)checkImageExistedOfFriend:(Friends *)aFriend
{
    BOOL result = YES;
    if (aFriend.originalHeadImagePath == nil || aFriend.mediumHeadImagePath == nil) {
        result = NO;
        return result;
    }
    UIImage *image;
    if (aFriend.originalHeadImagePath != nil) {
        image = [UIImage imageWithContentsOfFile:aFriend.originalHeadImagePath];
        NSData *imgData = UIImagePNGRepresentation(image);
        if ([imgData length] == 0) {
            result = NO;
        }
    }
    if (aFriend.mediumHeadImagePath != nil) {
        image = [UIImage imageWithContentsOfFile:aFriend.mediumHeadImagePath];
        NSData *imgData = UIImagePNGRepresentation(image);
        if ([imgData length] == 0) {
            result = NO;
        }
    ...
    /*
    if (aFriend.thumbnailsHeadImagePath != nil) {
        image = [UIImage imageWithContentsOfFile:aFriend.thumbnailsHeadImagePath];
        NSData *imgData = UIImagePNGRepresentation(image);
        if ([imgData length] > 0) {
            result = YES;
        }
