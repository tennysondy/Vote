...
@property (strong, nonatomic) VotesUserSetting *aVoteUS;
...
- (void)viewDidLoad
{
    ...
    [CoreDataHelper sharedDatabase:^(UIManagedDocument *database) {
        ...
        if ([self.aVote.isEnd boolValue] == YES) {
            ...
            //[self.navigationItem.rightBarButtonItem setTitleTextAttributes:@{NSForegroundColorAttributeName: UIColorFromRGB(0xCCCCCC), NSFontAttributeName:[UIFont systemFontOfSize:15.0f]} forState:UIControlStateNormal];
        }
        
        self.aVoteUS = [VotesUserSetting fetchVoteUserSetting:self.aVote.voteID withContext:self.managedObjectContext];
        maxChoice = [self.aVote.maxChoice intValue];
        isNotification = [self.aVoteUS.notification boolValue];
        isDeleteForever = [self.aVoteUS.deleteForever boolValue];
...
- (IBAction)send:(id)sender {
    if ([self.checkMarkSet count] >= 1) {
        ...
        [manager POST:votesURL parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            {
                if ([nav isKindOfClass:[VoteChooseOptionsTableViewController class]])
                {
...
- (void)handleSwitchValueChanged:(id)sender
{
    ...
    if (switchCtrl.tag == COTVC_SWITCH_NOTIFICATION_TAG) {
        ...
        [VotesUserSetting modifyVoteUserSettingNotificationFlag:isNotification withVoteId:self.aVote.voteID withContext:self.managedObjectContext];
        //向服务器发送设置消息通知
        
    } else {
        isDeleteForever = switchCtrl.on;
        [VotesUserSetting modifyVoteUserSettingDeleteForeverFlag:isDeleteForever withVoteId:self.aVote.voteID withContext:self.managedObjectContext];
