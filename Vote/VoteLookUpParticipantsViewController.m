...
{
    VoteLookUpParticipantsCell *cell;
    if ([self.identifier isEqualToString:@"Look Up Participants"]) {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"Participants" forIndexPath:indexPath];
    } else {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"Voters" forIndexPath:indexPath];
    }
    if (indexPath.row == [self.participants count]) {
        ...
        NSString *participantName = [name objectForKey:SERVER_USERNAME];
        NSString *screenname = [name objectForKey:SERVER_SCREENNAME];
