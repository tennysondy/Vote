...
+ (void)modifyUsers:(Users *)aUser withData:(NSDictionary *)data withManagedObjectContext:(NSManagedObjectContext *)context withQueue:(NSOperationQueue *)queue
{
    ...
    if ([Users checkImageExistedOfUser:aUser] == NO) {
        aUser.headImageLastUpdateTag = [NSNumber numberWithInteger:0];
    }
...
+ (void)batchDownloadHeadImage:(Users *)aUser withManagedObjectContext:(NSManagedObjectContext *)context withQueue:(NSOperationQueue *)queue
{
...
+ (BOOL)checkImageExistedOfUser:(Users *)aUser
{
    BOOL result = YES;
    if (aUser.originalHeadImagePath == nil || aUser.mediumHeadImagePath == nil || aUser.thumbnailsHeadImagePath == nil) {
        result = NO;
        return result;
    }
    UIImage *image;
    if (aUser.originalHeadImagePath != nil) {
        image = [UIImage imageWithContentsOfFile:aUser.originalHeadImagePath];
        NSData *imgData = UIImagePNGRepresentation(image);
        if ([imgData length] == 0) {
            result = NO;
        }
    }
    if (aUser.mediumHeadImagePath != nil) {
        image = [UIImage imageWithContentsOfFile:aUser.mediumHeadImagePath];
        NSData *imgData = UIImagePNGRepresentation(image);
        if ([imgData length] == 0) {
            result = NO;
        }
    }
    if (aUser.thumbnailsHeadImagePath != nil) {
        image = [UIImage imageWithContentsOfFile:aUser.thumbnailsHeadImagePath];
        NSData *imgData = UIImagePNGRepresentation(image);
        if ([imgData length] == 0) {
            result = NO;
        }
