...
+ (void)configureBasicCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath withBusinessInfo:(NSDictionary *)businessInfo
{
    ...
    if ([cell.contentView viewWithTag:BDH_S_PHOTO_TAG] == nil) {
...
+ (void)configureAddressCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath withBusinessInfo:(NSDictionary *)businessInfo
{
    ...
    if ([cell.contentView viewWithTag:BDH_ADDRESS_IMG_TAG] == nil) {
        UIImage *image = [UIImage imageNamed:@"address.png"];
        imageView = [[UIImageView alloc] initWithFrame:CGRectMake(BDH_ADDR_IMG_COORDINATE_X, FRAME_HEIGHT(cell)/2-BDH_ADDR_IMG_HEIGHT/2, BDH_ADDR_IMG_WIDTH, BDH_ADDR_IMG_HEIGHT)];
        imageView.tag = BDH_ADDRESS_IMG_TAG;
        imageView.image = image;
        [cell.contentView addSubview:imageView];
    }
    if ([cell.contentView viewWithTag:BDH_ADDRESS_TEXT_TAG] == nil) {
        if (!((NSNull *)[businessInfo objectForKey:DIANPING_ADDRESS] == [NSNull null])) {
            ...
            addressLabel.tag = BDH_ADDRESS_TEXT_TAG;
...
+ (void)configureTelephoneCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath withBusinessInfo:(NSDictionary *)businessInfo
{
    ...
    if ([cell.contentView viewWithTag:BDH_TELEPHONE_IMG_TAG] == nil) {
        UIImage *image = [UIImage imageNamed:@"telephone.png"];
        imageView = [[UIImageView alloc] initWithFrame:CGRectMake(BDH_TELEPHONE_IMG_COORDINATE_X, FRAME_HEIGHT(cell)/2-BDH_TELEPHONE_IMG_HEIGHT/2, BDH_ADDR_IMG_WIDTH, BDH_TELEPHONE_IMG_HEIGHT)];
        imageView.tag = BDH_TELEPHONE_IMG_TAG;
        imageView.image = image;
        [cell.contentView addSubview:imageView];
    }
    if ([cell.contentView viewWithTag:BDH_TELEPHONE_TEXT_TAG] == nil) {
        if (!((NSNull *)[businessInfo objectForKey:DIANPING_TELEPHONE] == [NSNull null])) {
            ...
            telephoneLabel.tag = BDH_TELEPHONE_TEXT_TAG;
...
//团购
+ (void)configureDealsCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath withBusinessInfo:(NSDictionary *)businessInfo
{
    ...
    UIImageView *imageView;
    UILabel *lable;
    imageView = [[UIImageView alloc] initWithFrame:CGRectMake(BDH_DEALS_IMG_COORDINATE_X, FRAME_HEIGHT(cell)/2 - BDH_DEALS_IMG_HEIGHT/2, BDH_DEALS_IMG_WIDTH, BDH_DEALS_IMG_HEIGHT)];
    imageView.image = [UIImage imageNamed:@"tuan.png"];
    [cell.contentView addSubview:imageView];
    lable = [[UILabel alloc] initWithFrame:CGRectMake(BDH_DEALS_COORDINATE_X, BDH_DEALS_COORDINATE_Y, BDH_DEALS_WIDTH, FRAME_HEIGHT(cell)-20.0)];
    lable.text = [[[businessInfo objectForKey:DIANPING_DEALS] objectAtIndex:indexPath.row] objectForKey:DIANPING_DEALS_DESCR];
    lable.lineBreakMode = NSLineBreakByWordWrapping;
    lable.numberOfLines = 0;
    lable.font = [UIFont fontWithName:BDH_DEALS_FONT size:BDH_DEALS_FONT_SIZE];
    [cell.contentView addSubview:lable];
    cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
}

//优惠券
+ (void)configureCouponCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath withBusinessInfo:(NSDictionary *)businessInfo
{
    while ([[cell.contentView subviews] lastObject] != nil) {
        [[[cell.contentView subviews] lastObject] removeFromSuperview];
    }
    UIImageView *imageView;
    UILabel *lable;
    imageView = [[UIImageView alloc] initWithFrame:CGRectMake(BDH_COUPON_IMG_COORDINATE_X, FRAME_HEIGHT(cell)/2 - BDH_COUPON_IMG_HEIGHT/2, BDH_COUPON_IMG_WIDTH, BDH_COUPON_IMG_HEIGHT)];
    imageView.image = [UIImage imageNamed:@"coupon.png"];
    [cell.contentView addSubview:imageView];
    lable = [[UILabel alloc] initWithFrame:CGRectMake(BDH_COUPON_COORDINATE_X, BDH_COUPON_COORDINATE_Y, BDH_COUPON_WIDTH, FRAME_HEIGHT(cell)-20.0)];
    lable.text = [businessInfo objectForKey:DIANPING_COUPON_DESCR];
    lable.lineBreakMode = NSLineBreakByWordWrapping;
    lable.numberOfLines = 0;
    lable.font = [UIFont fontWithName:BDH_COUPON_FONT size:BDH_COUPON_FONT_SIZE];
    [cell.contentView addSubview:lable];
    cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
}
